...
from src.api import result_processor
...
async def get_result(job_id: str):
    try:
        raw_result = await model_client.get_result(job_id)
        processed_result = result_processor.process_result(raw_result)
...
@router.get("/jobs/{job_id}/raw_result")
async def get_result(job_id: str):
    try:
        raw_result = await model_client.get_result(job_id)
        return Response(
            content=json.dumps(raw_result, indent=2, ensure_ascii=False),
